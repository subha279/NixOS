...
// BatteryService
// ----------------------------------------------------------------
// Thin, defensive wrapper around Quickshell's UPower service.
//
// Why a wrapper?
//   * UPower reports `percentage` as 0..1 in some builds and
//     0..100 in others. We normalise to 0..100 once, here.
//   * `displayDevice` can be null on desktops with no battery.
//   * PowerProfiles may not exist (no power-profiles-daemon).
//     Every access is guarded so the shell never dies.
//
// Everything the UI needs is exposed as a plain property, so the
// popup and the bar module stay dumb.
// ================================================================
...
Singleton {
    ...
    readonly property var device:
        (typeof UPower !== "undefined" && UPower.displayDevice)
    ...
    readonly property bool available:
        root.device !== null
        && root.device.isPresent === true
        && root.device.isLaptopBattery !== false

    // 0..100, always.
    readonly property real percent:
        root.device ? root.normalise(root.device.percentage) : 0

    readonly property int percentInt: Math.round(root.percent)

    // UPowerDeviceState numeric values:
    //   0 Unknown  1 Charging  2 Discharging  3 Empty
    //   4 FullyCharged  5 PendingCharge  6 PendingDischarge
    readonly property int state:
        root.device ? root.device.state : 0

    readonly property bool charging:
        root.state === 1 || root.state === 5

    readonly property bool full:
        root.state === 4

    readonly property bool discharging:
        root.state === 2 || root.state === 6

    // AC connected (either explicitly charging, full, or UPower
    // says we are not running on battery).
    readonly property bool onAc:
        root.charging
        || root.full
        || (typeof UPower !== "undefined"
            && UPower.onBattery === false)

    readonly property int secondsToEmpty:
        root.device && root.device.timeToEmpty
            ? root.device.timeToEmpty
            : 0

    readonly property int secondsToFull:
        root.device && root.device.timeToFull
            ? root.device.timeToFull
            : 0

    // Watts. Negative means draining in some builds — take abs.
    readonly property real changeRate:
        root.device && root.device.changeRate
            ? Math.abs(root.device.changeRate)
            : 0

    readonly property real health:
        (root.device && root.device.healthSupported
            && root.device.healthPercentage)
            ? root.normalise(root.device.healthPercentage)
            : -1
    ...
    readonly property bool low:
        root.available && !root.onAc
        && root.percentInt <= root.lowThreshold

    readonly property bool critical:
        root.available && !root.onAc
        && root.percentInt <= root.criticalThreshold
    ...
    readonly property var allDevices:
        (typeof UPower !== "undefined" && UPower.devices)
    ...
    readonly property bool profilesAvailable:
        typeof PowerProfiles !== "undefined"

    readonly property int profile:
        root.profilesAvailable ? PowerProfiles.profile : 1

    readonly property bool hasPerformance:
        root.profilesAvailable
            ? PowerProfiles.hasPerformanceProfile !== false
            : false

    readonly property string degradationReason:
        (root.profilesAvailable && PowerProfiles.degradationReason)
            ? String(PowerProfiles.degradationReason)
            : ""
    ...
    readonly property string stateLabel: {
        ...
        if (root.charging)
            return root.secondsToFull > 0
                ? root.formatTime(root.secondsToFull)
                    + " until full"
                : "Charging"

        if (root.discharging)
            return root.secondsToEmpty > 0
                ? root.formatTime(root.secondsToEmpty)
                    + " remaining"
                : "On battery"
    ...
    readonly property string icon: {
        ...
        const idx = Math.max(
            0, Math.min(10, Math.round(root.percent / 10)))

        return root.charging || root.full
            ? root.chargeIcons[idx]
            : root.dischargeIcons[idx]
    }
    ...
    function deviceIcon(dev) {
        ...
        try {
            ...
            if (t === UPowerDeviceType.Headphones
                || t === UPowerDeviceType.Headset)
                return "\udb81\udcd0"
    ...
    function rebuildPeripherals() {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            // Skip the laptop battery + the synthetic display
            // device + AC adapters — they have no useful percent.
            if (dev === root.device)
                continue
    ...
    // Reconcile a ListModel in place so ListView add/remove
    // transitions actually run instead of everything flashing.
    ...
    function openPowerSettings() {
        ...
        root.launch([
            ...
            + "|| command -v xfce4-power-manager-settings "
            + ">/dev/null && xfce4-power-manager-settings "
    ...
    // UPower is event driven, but peripheral percentages update
    // lazily — a slow tick keeps them honest without any cost.
    Timer {
        interval: 30000
        running: true
        repeat: true
