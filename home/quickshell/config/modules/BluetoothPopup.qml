    ...
    Column {
        ...
        Item {
            ...
            Text {
                ...
                text: popup.svc.discovering
                    ? "scanning…"
                    : popup.svc.deviceModel.count + " found"
        ...
        Item {
            ...
            ListView {
                ...
                delegate: Components.ListRow {
                    ...
                    trailing: devRow.battery >= 0
                        ? devRow.battery + "%"
                        : devRow.connected
                            ? "\udb80\udd34"
                            : ""

                    trailingColor: devRow.battery >= 0
                        ? (devRow.battery < 20
                            ? Core.Theme.danger
                    ...
                    onContextRequested: function(mx, my) {
                        ...
                        items.push({
                            icon: devRow.connected
                                ? "\udb80\udcb2"
                                : "\udb80\udcb1",

                            label: devRow.connected
                                ? "Disconnect"
                                : "Connect",

                            action: function() {
                                popup.svc.toggleDevice(
                                    devRow.address)
                            }
                        })
                        ...
                        if (devRow.paired) {
                            items.push({
                                ...
                                label: devRow.trusted
                                    ? "Untrust device"
                                    : "Trust device",

                                action: function() {
                                    popup.svc.setTrusted(
                                        devRow.address,
                                        !devRow.trusted)
                                }
                            })
                        ...
                        items.push({
                            ...
                            label: devRow.blocked
                                ? "Unblock"
                                : "Block",

                            action: function() {
                                popup.svc.setBlocked(
                                    devRow.address,
                                    !devRow.blocked)
                            }
                        })
