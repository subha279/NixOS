...
let
  ...
  themeToTmux =
    ...
    let
      ...
      # themes.nix names the clock colours the way Quickshell's Theme.qml aliases
      # them, so the same mapping has to be applied here or the tmux clock comes
      # out a different colour from the bar clock reading the same key.
      clockColors = {
        foreground = colors.text;
    ...
    ''
      ...
      # ========================================================
      #
      # Only appearance lives here. Behaviour is in home/tmux/default.nix, so
      # switching theme can never change a keybinding.
      #
      # The rounded caps are U+E0B6 and U+E0B4, the same pair the Aurora starship
      # prompt uses, so the tmux status line and the shell prompt round off the
      # same way. They need the Nerd Font that global.fonts.terminal already is.
      ...
      # --------------------------------------------------------
      #
      # accentForeground is the near-background tone the theme pairs with accent,
      # which is what keeps the session name legible on an accent fill.
      ...
      # --------------------------------------------------------
      # Clock and prefix indicator
      # --------------------------------------------------------
      #
      # The hour/separator/minute colours come from global.ui.clock, the same
      # three keys the Quickshell bar clock reads.
      ...
      # --------------------------------------------------------
      #
      # accentMuted rather than accent: selected text keeps the theme's normal
      # foreground on top of it, and a full-strength accent fill drops that to an
      # unreadable contrast ratio. Same reasoning as kitty's selection colours.
      ...
      # --------------------------------------------------------
      #
      # Rounded to match every other Aurora surface.
  ...
  home.file.".local/bin/aurora-theme" = {
    ...
    text = ''
      ...
      if [[ ! -f "$theme_tmux" ]]; then
        ...
        echo \
          "Aurora: generated Tmux theme not found: $theme_id" \
          >&2
      ...
      # ========================================================
      # Reload Running Tmux Servers
      # ========================================================
      #
      # source-file re-reads only the theme, which is why appearance and
      # behaviour are generated into separate files: a running session picks up
      # new colours without its keybindings being torn down and rebuilt.
      #
      # `has-session` is the cheap way to ask whether a server is even up;
      # without it tmux would start one just to be told to reload.
      #
      # ========================================================
