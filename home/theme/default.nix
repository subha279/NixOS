...
let
  ...
  #   Fuzzel
  #
  # Only visual/theme values are generated here.
  #
  # Layout and behavior remain in:
  #
  #   ~/NixOS/home/fuzzel/fuzzel.ini
  #
  # ============================================================
  ...
  themeToFuzzel =
    ...
    ''
      ...
      placeholder=${colors.textMuted}ff

      input=${colors.text}ff

      match=${colors.accent}ff

      selection=${colors.surfaceActive}e8

      selection-text=${colors.text}ff

      selection-match=${colors.accentHover}ff

      counter=${colors.textMuted}ff

      border=${colors.borderFocus}ff


      [border]

      width=${toString ui.borderWidth}
  ...
  # ============================================================
  # GENERATE FUZZEL FILES
  # ============================================================

  fuzzelThemeFiles = lib.genAttrs themeNames (themeId: {
    text = themeToFuzzel themeId;
  });
  ...
  generatedFuzzelFiles = lib.mapAttrs' (
    themeId: file: lib.nameValuePair "aurora/themes/${themeId}.fuzzel.conf" file
  ) fuzzelThemeFiles;
...
in
{
  ...
  xdg.configFile = {
    ...
    # --------------------------------------------------------
    # Fuzzel-readable theme list
    # --------------------------------------------------------

    "aurora/themes.list".text = themeList + "\n";

    # --------------------------------------------------------
    # Main Fuzzel configuration
    # --------------------------------------------------------
    #
    # The configuration itself remains in:
    #
    #   ~/NixOS/home/fuzzel/fuzzel.ini
    #
    # This file contains layout/behavior.
    #
    # Its colors come from:
    #
    #   ~/.config/aurora/active-fuzzel.conf
    #
    # We install it here instead of importing
    # home/fuzzel as a Home Manager module.
    #
    # --------------------------------------------------------
  ...
  home.activation.initializeAuroraTheme = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    active_fuzzel="$theme_dir/active-fuzzel.conf"
    ...
    if [[ -f "$theme_dir/themes/$selected.fuzzel.conf" ]]; then

      ln -sfn \
        "$theme_dir/themes/$selected.fuzzel.conf" \
        "$active_fuzzel"

    else

      ln -sfn \
        "$theme_dir/themes/aurora.fuzzel.conf" \
        "$active_fuzzel"
  ...
  home.file.".local/bin/aurora-theme" = {
    ...
    text = ''
      ...
      ACTIVE_FUZZEL="$CONFIG_DIR/active-fuzzel.conf"
      ...
      theme_fuzzel="$THEME_DIR/$theme_id.fuzzel.conf"
      ...
      if [[ ! -f "$theme_fuzzel" ]]; then

        echo \
          "Aurora: generated Fuzzel theme not found: $theme_id" \
          >&2
